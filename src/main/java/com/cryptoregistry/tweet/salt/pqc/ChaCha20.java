...
class Salsa20Engine {
	...
	private final static int[] TAU_SIGMA = littleEndianToInt(
	...
	protected static int littleEndianToInt(byte[] bs, int off) {
		int n = bs[off] & 0xff;
		n |= (bs[++off] & 0xff) << 8;
		n |= (bs[++off] & 0xff) << 16;
		n |= bs[++off] << 24;
	...
	protected static void littleEndianToInt(byte[] bs, int bOff, int[] ns,
			int nOff, int count) {
