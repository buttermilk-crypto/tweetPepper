...
class ErrorCorrection {
	...
	int f(int[] v, int off0, int off1, int x) {
		int xit, t, r, b;
	...
	void helpRec(short[] c, short[] v, byte[] seed, byte nonce) {
		byte[] iv = new byte[8];
		// iv[7] = nonce;
		iv[0] = nonce;

		byte[] rand = new byte[32];
		new ChaCha20().process(seed, iv, rand, 0, rand.length);
	...
	short LDDecode(int xi0, int xi1, int xi2, int xi3) {
		int t;
	...
	void rec(byte[] key, short[] v, short[] c) {
		Arrays.fill(key, (byte) 0);

		int[] tmp = new int[4];
		for (int i = 0; i < 256; ++i) {
			tmp[0] = 16 * Params.Q + 8 * (int) v[0 + i] - Params.Q
					* (2 * c[0 + i] + c[768 + i]);
			tmp[1] = 16 * Params.Q + 8 * (int) v[256 + i] - Params.Q
					* (2 * c[256 + i] + c[768 + i]);
			tmp[2] = 16 * Params.Q + 8 * (int) v[512 + i] - Params.Q
					* (2 * c[512 + i] + c[768 + i]);
			tmp[3] = 16 * Params.Q + 8 * (int) v[768 + i] - Params.Q
					* (c[768 + i]);
